...
use App\Http\Controllers\DoctorConsultationImagingOrderController;
use App\Http\Controllers\DoctorConsultationLabOrderController;
...
use App\Http\Controllers\FinanceBillingSummaryController;
use App\Http\Controllers\FinanceDebtorController;
...
use App\Http\Controllers\InsurancePolicyImportController;
use App\Http\Controllers\InsurancePolicyItemController;
...
use App\Http\Controllers\LabOrderItemConsumableController;
...
Route::middleware(['auth', 'verified', 'ensure.active.branch'])->group(function (): void {
    ...
    Route::middleware('support.only')
        ...
        ->group(function (): void {
            ...
            Route::get('facilities/{tenant}/audit-log', [FacilityManagerController::class, 'auditLog'])->name('facilities.audit-log');
    ...
    Route::get('data-upload/facility-services/template', [DataUploadController::class, 'facilityServiceTemplate'])->name('data-upload.facility-services.template');
    Route::post('data-upload/facility-services/import', [DataUploadController::class, 'importFacilityServices'])->name('data-upload.facility-services.import');
    Route::post('data-upload/facility-service-imports/{dataImport}/confirm', [DataUploadController::class, 'confirmFacilityServiceImport'])->name('data-upload.facility-service-imports.confirm');
    ...
    Route::post('insurance-packages/{insurance_package}/policies', [InsurancePolicyController::class, 'store'])->name('insurance-packages.policies.store');
    Route::patch('insurance-packages/{insurance_package}/policies/{insurance_policy}', [InsurancePolicyController::class, 'update'])->name('insurance-packages.policies.update');
    Route::delete('insurance-packages/{insurance_package}/policies/{insurance_policy}', [InsurancePolicyController::class, 'destroy'])->name('insurance-packages.policies.destroy');
    Route::post('insurance-packages/{insurance_package}/policies/{insurance_policy}/items', [InsurancePolicyItemController::class, 'store'])->name('insurance-packages.policies.items.store');
    Route::patch('insurance-packages/{insurance_package}/policies/{insurance_policy}/items/{insurance_policy_item}', [InsurancePolicyItemController::class, 'update'])->name('insurance-packages.policies.items.update');
    Route::delete('insurance-packages/{insurance_package}/policies/{insurance_policy}/items/{insurance_policy_item}', [InsurancePolicyItemController::class, 'destroy'])->name('insurance-packages.policies.items.destroy');
    Route::get('insurance-packages/{insurance_package}/policies/{insurance_policy}/template', [InsurancePolicyImportController::class, 'template'])->name('insurance-packages.policies.template');
    Route::post('insurance-packages/{insurance_package}/policies/{insurance_policy}/import', [InsurancePolicyImportController::class, 'import'])->name('insurance-packages.policies.import');
    Route::post('insurance-policy-imports/{dataImport}/confirm', [InsurancePolicyImportController::class, 'confirm'])->name('insurance-policy-imports.confirm');
    ...
    Route::post('finance/opd-payments/{visit}/discounts', [BillingDiscountController::class, 'store'])->name('finance.opd-payments.discounts.store');
    Route::post('finance/opd-payments/{visit}/discounts/{discount}/approve', [BillingDiscountController::class, 'approve'])->name('finance.opd-payments.discounts.approve');
    Route::post('finance/opd-payments/{visit}/discounts/{discount}/reverse', [BillingDiscountController::class, 'reverse'])->name('finance.opd-payments.discounts.reverse');
    Route::get('finance/insurance-invoices', [InsuranceCompanyInvoiceController::class, 'index'])->name('finance.insurance-invoices.index');
    Route::post('finance/insurance-invoices', [InsuranceCompanyInvoiceController::class, 'store'])->name('finance.insurance-invoices.store');
    Route::get('finance/insurance-invoices/{invoice}', [InsuranceCompanyInvoiceController::class, 'show'])->name('finance.insurance-invoices.show');
    Route::post('finance/insurance-invoices/{invoice}/payments', [InsuranceCompanyInvoiceController::class, 'storePayment'])->name('finance.insurance-invoices.payments.store');
    Route::get('finance/deposits', [BillingDepositController::class, 'index'])->name('finance.deposits.index');
    Route::post('finance/deposits', [BillingDepositController::class, 'store'])->name('finance.deposits.store');
    Route::post('finance/deposits/{deposit}/apply', [BillingDepositController::class, 'apply'])->name('finance.deposits.apply');
    Route::get('finance/debtors', [FinanceDebtorController::class, 'index'])->name('finance.debtors.index');
    Route::get('finance/debtors/{billing}', [FinanceDebtorController::class, 'show'])->name('finance.debtors.show');
    Route::post('finance/debtors/{billing}/write-offs', [FinanceDebtorController::class, 'storeWriteOff'])->name('finance.debtors.write-offs.store');
    Route::post('finance/debtors/{billing}/write-offs/{writeOff}/approve', [FinanceDebtorController::class, 'approveWriteOff'])->name('finance.debtors.write-offs.approve');
    Route::post('finance/debtors/{billing}/write-offs/{writeOff}/reverse', [FinanceDebtorController::class, 'reverseWriteOff'])->name('finance.debtors.write-offs.reverse');
    Route::get('finance/billing-summary', [FinanceBillingSummaryController::class, 'index'])->name('finance.billing-summary.index');
    Route::get('prescriptions/{prescription}/print', [PrescriptionPrintController::class, 'show'])->name('prescriptions.print');
    Route::post('visits/{visit}/lab-orders', [VisitOrderController::class, 'storeLabOrder'])->name('visits.lab-orders.store');
    Route::patch('visits/{visit}/lab-orders/{labOrder}', [VisitOrderController::class, 'updateLabOrder'])->name('visits.lab-orders.update');
    Route::delete('visits/{visit}/lab-orders/{labOrder}', [VisitOrderController::class, 'destroyLabOrder'])->name('visits.lab-orders.destroy');
    Route::delete('visits/{visit}/lab-orders/{labOrder}/items/{labOrderItem}', [VisitOrderController::class, 'destroyLabOrderItem'])->name('visits.lab-orders.items.destroy');
    Route::post('visits/{visit}/imaging-orders', [VisitOrderController::class, 'storeImagingOrder'])->name('visits.imaging-orders.store');
    ...
    Route::post('doctors/consultations/{visit}/lab-orders', [DoctorConsultationLabOrderController::class, 'store'])->name('doctors.consultations.lab-orders.store');
    Route::post('doctors/consultations/{visit}/imaging-orders', [DoctorConsultationImagingOrderController::class, 'store'])->name('doctors.consultations.imaging-orders.store');
    ...
    Route::get('laboratory/order-items/{labOrderItem}', [LaboratoryWorklistController::class, 'show'])->name('laboratory.order-items.show');
    Route::post('laboratory/order-items/{labOrderItem}/collect-sample', [LabResultWorkflowController::class, 'collectSample'])->name('laboratory.order-items.collect-sample');
    Route::post('laboratory/order-items/{labOrderItem}/receive', [LabResultWorkflowController::class, 'receive'])->name('laboratory.order-items.receive');
    Route::post('laboratory/order-items/{labOrderItem}/results', [LabResultWorkflowController::class, 'store'])->name('laboratory.order-items.results.store');
    Route::post('laboratory/order-items/{labOrderItem}/correct', [LabResultWorkflowController::class, 'correct'])->name('laboratory.order-items.correct');
    Route::post('laboratory/order-items/{labOrderItem}/review', [LabResultWorkflowController::class, 'review'])->name('laboratory.order-items.review');
    Route::post('laboratory/order-items/{labOrderItem}/approve', [LabResultWorkflowController::class, 'approve'])->name('laboratory.order-items.approve');
    Route::get('laboratory/order-items/{labOrderItem}/print', [LabResultPrintController::class, 'show'])->name('laboratory.order-items.print');
    Route::get('laboratory/order-items/{labOrderItem}/consumables', [LabOrderItemConsumableController::class, 'show'])->name('laboratory.order-items.consumables.show');
    Route::post('laboratory/order-items/{labOrderItem}/consumables', [LabOrderItemConsumableController::class, 'store'])->name('laboratory.order-items.consumables.store');
    Route::delete('laboratory/order-items/{labOrderItem}/consumables/{labOrderItemConsumable}', [LabOrderItemConsumableController::class, 'destroy'])->name('laboratory.order-items.consumables.destroy');
