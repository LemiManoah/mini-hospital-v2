...
it('shows inventory consumable defaults on the dedicated consumables page', function (): void {
    [$branch, $user, $orderItem] = createLaboratoryWorklistContext();
    ...
    $response = $this->withSession(['active_branch_id' => $branch->id])
        ->actingAs($user)
        ->get(route('laboratory.order-items.consumables.show', $orderItem));

    $response->assertOk()
        ->assertInertia(fn (AssertableInertia $page): AssertableInertia => $page
            ->component('laboratory/order-item-consumables')
...
it('shows a computed patient age on queue cards when date of birth is available', function (): void {
    ...
    $orderItem->order?->visit?->patient?->forceFill([
    ...
    $response->assertOk()
        ->assertInertia(fn (AssertableInertia $page): AssertableInertia => $page
            ...
            ->where('orders.data.0.visit.patient.display_age', 10)
            ->where('orders.data.0.visit.patient.display_age_units', 'year'));
});
...
it('records and removes consumable usage while syncing actual cost and stock', function (): void {
    ...
    $storeResponse = $this->withSession(['active_branch_id' => $branch->id])
        ->actingAs($user)
        ->post(route('laboratory.order-items.consumables.store', $orderItem), $payload);

    $storeResponse->assertRedirectToRoute('laboratory.order-items.consumables.show', $orderItem);
    $storeResponse->assertSessionHas('success', 'Consumable usage recorded successfully.');

    $orderItem->refresh();

    expect((float) $orderItem->actual_cost)->toBe(3000.0)
        ->and($orderItem->status->value)->toBe('in_progress');
    ...
    $usage = DB::table('lab_order_item_consumables')
    ...
    $deleteResponse = $this->withSession(['active_branch_id' => $branch->id])
        ->actingAs($user)
        ->delete(route('laboratory.order-items.consumables.destroy', [
            'labOrderItem' => $orderItem,
            'labOrderItemConsumable' => $usage->id,
        ]));

    $deleteResponse->assertRedirectToRoute('laboratory.order-items.consumables.show', $orderItem);
    ...
    expect((float) $orderItem->fresh()->actual_cost)->toBe(0.0)
