...
import {
    type LaboratoryOrderItemPageProps,
...
export default function LaboratoryOrderItemShow({
    labOrderItem,
    labReleasePolicy,
    paymentBlockMessage,
}: LaboratoryOrderItemPageProps) {
    const patient = labOrderItem.order?.visit?.patient ?? null;
    const resultEntry: LaboratoryResultEntry | null =
        labOrderItem.resultEntry ?? labOrderItem.result_entry ?? null;
    const resultValues = resultEntry?.values ?? [];
    const resultOptions = labOrderItem.test?.result_options ?? [];
    const resultParameters = labOrderItem.test?.result_parameters ?? [];
    const resultType = labOrderItem.test?.result_capture_type ?? 'free_entry';
    const isApproved = labOrderItem.workflow_stage === 'approved';
    const canRelease =
        labOrderItem.workflow_stage === 'result_entered' ||
        labOrderItem.workflow_stage === 'reviewed' ||
        labOrderItem.workflow_stage === 'approved';
    ...
    const releaseSubmitUrl =
        !requiresApproval && !isApproved
            ? `/laboratory/order-items/${labOrderItem.id}/review`
            : `/laboratory/order-items/${labOrderItem.id}/approve`;
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: pageTitle,
            href: `/laboratory/order-items/${labOrderItem.id}`,
        },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head
                title={`${pageTitle} ${labOrderItem.test?.test_name ?? ''}`}
            />
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                {labOrderItem.result_visible ? (
                                    <Button type="button" asChild>
                                        <a
                                            href={`/laboratory/order-items/${labOrderItem.id}/print`}
                                            target="_blank"
                                            rel="noreferrer"
                                        >
                                            Download PDF Result
                                ...
                                <Button variant="outline" asChild>
                                    <Link
                                        href={`/laboratory/order-items/${labOrderItem.id}/consumables`}
                                    >
                                        Consumables
                                    </Link>
                                </Button>
                            ...
                            <CardContent className="flex flex-col gap-4">
                                <form
                                    onSubmit={(event) => {
                                        ...
                                        resultForm.post(
                                            correctionMode
                                                ? `/laboratory/order-items/${labOrderItem.id}/correct`
                                                : `/laboratory/order-items/${labOrderItem.id}/results`,
                                            { preserveScroll: true },
                                        );
                                    }}
