...
import {
    ...
    type LaboratoryOrderItemConsumablesPageProps,
...
export default function LaboratoryOrderItemConsumablesPage({
    labOrderItem,
    consumableOptions,
}: LaboratoryOrderItemConsumablesPageProps) {
    const patient = labOrderItem.order?.visit?.patient ?? null;
    const variance =
        (labOrderItem.price ?? 0) - (labOrderItem.actual_cost ?? 0);
    const pageTitle = 'Consumables';
    const isResultReleased = labOrderItem.approved_at !== null;
                            ...
                            <CardContent className="flex flex-col gap-4">
                                <form
                                    onSubmit={(event) => {
                                        ...
                                        consumableForm.post(
                                            `/laboratory/order-items/${labOrderItem.id}/consumables`,
                                            { preserveScroll: true },
                                        );
                                    }}
                                ...
                                {(labOrderItem.consumables ?? []).length ? (
                                    <div className="flex flex-col gap-3">
                                        {labOrderItem.consumables?.map(
                                            (usage) => (
                                                <div
                                                    key={usage.id}
                                                ...
                                                >
                                                    <div className="flex flex-col gap-3 lg:flex-row lg:justify-between">
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={() =>
                                                                router.delete(
                                                                    `/laboratory/order-items/${labOrderItem.id}/consumables/${usage.id}`,
                                                                    {
                                                                        preserveScroll: true,
                                                                    },
                                                                )
                    ...
                    <div className="flex flex-col gap-6">
                        <Card>
                            ...
                            <CardContent className="flex flex-col gap-3 text-sm">
                                <SummaryRow
                                    label="Billed Price"
                                    value={formatMoney(labOrderItem.price)}
                                />
                                <SummaryRow
                                    label="Actual Cost"
                                    value={formatMoney(
                                        labOrderItem.actual_cost,
                                    )}
                                />
                                <SummaryRow
                                    label="Variance"
