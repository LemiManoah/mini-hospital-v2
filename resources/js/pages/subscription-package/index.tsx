...
import { type SubscriptionPackage, type SubscriptionPackageIndexPageProps } from '@/types/subscription-package';
...
export default function SubscriptionPackageIndex({ packages, filters }: SubscriptionPackageIndexPageProps) {
    const rows: SubscriptionPackage[] = Array.isArray(packages) ? packages : (packages.data ?? []);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mt-4 mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-4">
                ...
                <Button asChild className="shadow-sm border border-zinc-200 dark:border-zinc-800 shrink-0">
                    <Link href={SubscriptionPackageController.create.url()} className="gap-2">
            ...
            <div className="m-2 overflow-x-auto rounded border p-4 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
                <Table className="min-w-[1000px]">
                    <TableHeader>
                        <TableRow>
                            <TableHead className="uppercase tracking-wider text-xs font-semibold">Package Name</TableHead>
                            <TableHead className="uppercase tracking-wider text-xs font-semibold text-center">Users</TableHead>
                            <TableHead className="uppercase tracking-wider text-xs font-semibold text-center">Price</TableHead>
                            <TableHead className="uppercase tracking-wider text-xs font-semibold text-center">Status</TableHead>
                            ...
                            rows.map((pkg) => (
                                <TableRow key={pkg.id} className="group transition-colors">
                                    ...
                                    <TableCell className="text-center font-mono font-bold text-zinc-700 dark:text-zinc-300">
                                        {Number(pkg.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <span className="inline-flex items-center rounded-full border border-zinc-200 bg-zinc-100 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-tight text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                                            {pkg.status}
                                        </span>
                                    </TableCell>
                                    <TableCell className="text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            >
                                                <Link href={SubscriptionPackageController.edit.url({ subscription_package: pkg.id })}>
                                                    Edit
                                                </Link>
                                            </Button>
                                            ...
                                            <DeleteConfirmationModal
                                                ...
                                                action={SubscriptionPackageController.destroy.form({ subscription_package: pkg.id })}
                ...
                {(!Array.isArray(packages) && packages.links?.length > 3) ? (
                    <div className="mt-4">
                        <Pagination>
                            <PaginationContent>
                                <PaginationItem>
                                    <PaginationPrevious href={packages.prev_page_url ?? undefined} />
                                </PaginationItem>
                                ...
                                <PaginationItem>
                                    <PaginationNext href={packages.next_page_url ?? undefined} />
                                </PaginationItem>
                            </PaginationContent>
                        </Pagination>
                    </div>
