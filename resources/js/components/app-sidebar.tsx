...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const tenantName = (user.tenant as { name?: string } | null | undefined)?.name
  const activeBranchName = (user.active_branch as { name?: string } | null | undefined)?.name
  const userRoles = Array.isArray(user.roles) ? user.roles : []
  const canSwitchFacility = Boolean(user.is_support) || userRoles.includes("super_admin")
        ...
        <SidebarMenu>
          <SidebarMenuItem className="px-2 py-1">
            <div className="flex flex-col">
              <span className="truncate text-sm font-semibold">{tenantName ?? "Mini Hospital"}</span>
              <span className="truncate text-xs text-muted-foreground">{activeBranchName ?? "No active branch selected"}</span>
            </div>
          </SidebarMenuItem>
        </SidebarMenu>
      </SidebarHeader>
